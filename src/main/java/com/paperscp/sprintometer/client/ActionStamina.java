...
public class ActionStamina {
    ...
    private static byte i6 = 0; // Check Enabled Delay
    private static byte bb2 = 1; // Check Enabled Cache

    private static final Identifier sprintDebuffIdentifier = new Identifier("sprintometer", "sprintable");
    ...
    public static void tick() {
        ...
        if (!staminaEligible()) {
            if (i6 == 0) {bb2 = configurator(7); i6 = 4; return;}
        ...
        if (i2 == 0) {
            ...
            i2 = configurator(2);
            if (Stamina <= 25 && (isSprinting || isJumping) && intToBoolean(configurator(8))) {
    ...
    private static void staminaDeduction(boolean isSprinting, boolean isJumping) {
        ...
        if (isSprinting) { Stamina = Stamina - configurator(4);i = configurator(1); } // Sprint Deduct

        if (isJumping) { Stamina = Stamina - configurator(5);i = configurator(1); } // Jump Deduct
    }
    ...
    private static void staminaRestoration(boolean isSprinting, boolean isJumping) {
        ...
        Stamina = Stamina + configurator(6);
        i3 = configurator(3);
        if (Stamina > 100) {
            Stamina = 100;
        }
    ...
    public static boolean staminaEligible() {
        ...
        return !(player.isCreative() || player.isSpectator() ||
                isRidingVehicle(player) || client.isPaused() ||
                configurator(7) == 0
        );
    }
    ...
    private static byte configurator(int ix) {
        ...
        //client.player.playSound(SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, 0.1f, 0.5F);
        ...
        switch (ix) {
            case 1:
                return (byte) configArray[0]; // Cooldown Delay - Delay between when the player stops jumping and when it starts restoring
            case 2:
                return (byte) configArray[1]; // Stamina Deduction Delay - How fast the stamina reduces while sprinting (Also controls the frequency of the warning ping)
            case 3:
                return (byte) configArray[2]; // Stamina Restoration Delay - How fast the stamina restores
            case 4:
                return (byte) configArray[3]; // Stamina Sprint Deduction Amount - How much the sprinting deducts
            case 5:
                return (byte) configArray[4]; // Stamina Jump Deduction Amount - How much the jumping deducts
            case 6:
                return (byte) configArray[5]; // Stamina Restoration Amount - How much stamina is restored every tick
            case 7:
                return (byte) ((boolean) configArray[6] ? 1 : 0); // Is Enabled? - Determines if SprintOMeter enabled or not
            case 8:
                return (byte) (SprintOMeterServer.sprintConfig.lowStaminaWarn ? 1 : 0); // Warning Ping Enabled? - Determines if the warning ping should play or not (This 'case' is the same as the singleplayer one as the warning ping is a completely client-sided feature.)
